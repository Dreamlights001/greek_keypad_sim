...
class GreekKeypadSimulator:
    ...
    def setup_keypad_ui(self):
        ...
        # [修改点1] 将特殊符号按键赋值给 self.extra_btn，以便后续更新文字
        self.extra_btn = self.create_func_btn(keypad_frame, "∂ / ∇", 0, 5, self.type_symbol_extra)
    ...
    def update_key_labels(self):
        ...
        for btn, data in self.buttons:
            if not self.is_latex_mode:
                ...
                btn.config(text=label, fg="black", bg="#fff", font=("Arial", 12))
            else:
                code = data[4] if self.is_shift_mode else data[3]
                btn.config(text=code, fg="blue", bg="#e8f0fe", font=("Consolas", 10))
        
        # [修改点2] 更新特殊符号键 (∂ / ∇) 的显示文字
        # 这样按 Shift 时，键帽也会跟着变成 ∇ 或 \nabla，看起来更直观
        if not self.is_latex_mode:
            # Unicode 模式
            label = "∇" if self.is_shift_mode else "∂"
            self.extra_btn.config(text=label, fg="black", bg="#fff", font=("Arial", 12))
        else:
            # LaTeX 模式
            label = r"\nabla" if self.is_shift_mode else r"\partial"
            self.extra_btn.config(text=label, fg="blue", bg="#e8f0fe", font=("Consolas", 9))
    ...
    def type_symbol_extra(self):
        ...
        if not self.is_latex_mode:
            ...
            if self.is_shift_mode:
                ...
                self.insert_text(r"\partial") # 偏导代码
